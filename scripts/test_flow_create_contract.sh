...
PARENT_RESPONSE=$(curl -s -X POST "${API_URL}/auth/register" \
  ...
  -d "{
    \"username\": \"parent_${RANDOM_SUFFIX}\",
    \"email\": \"parent_${RANDOM_SUFFIX}@example.com\",
...
CHILD_RESPONSE=$(curl -s -X POST "${API_URL}/auth/register" \
  -H "Content-Type: application/json" \
  -d "{
    \"username\": \"child_${RANDOM_SUFFIX}\",
    \"email\": \"child_${RANDOM_SUFFIX}@example.com\",
    \"password\": \"password123\",
    \"role\": \"child\",
    \"firstName\": \"Петр\",
    \"lastName\": \"Иванов\"
...
CONTRACT_RESPONSE=$(curl -v -X POST "${API_URL}/contracts/" \
  -H "Content-Type: application/json" \
...
echo -e "\nИмя пользователя родителя: parent_${RANDOM_SUFFIX}"
echo -e "Имя пользователя ребенка: child_${RANDOM_SUFFIX}"
echo -e "Токен родителя: ${PARENT_TOKEN}"
